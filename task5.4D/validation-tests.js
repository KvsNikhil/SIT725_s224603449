...
async function run() {
  ...
  await test({
    ...
    path: createPath,
    expected: 400,
    body: { ...makeValidBook("b102"), year: "bad" },
    tags: ["CREATE_FAIL", "TYPE"]
  });

  // ---- T08 Boundary (future year) ----
  await test({
    id: "T08",
    name: "Future year",
    method: "POST",
    path: createPath,
    expected: 400,
    body: { ...makeValidBook("b103"), year: 3000 },
    tags: ["CREATE_FAIL", "BOUNDARY", "TEMPORAL"]
  });

  // ---- T09 Length violation ----
  await test({
    id: "T09",
    name: "Short title",
    method: "POST",
    path: createPath,
    expected: 400,
    body: { ...makeValidBook("b104"), title: "a" },
    tags: ["CREATE_FAIL", "LENGTH"]
  });

  // ---- T10 Update validation ----
  await test({
    id: "T10",
